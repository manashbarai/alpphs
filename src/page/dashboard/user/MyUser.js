...
const MyUser = () => {
  ...
  return (
    <>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 p-7  ">
            ...
            <div>
              ...
              <input
                ...
                className="border border-red-600 rounded-md p-2 w-full"
                required
              />
            </div>
      ...
      <div className="px-4 lg:px-28 mt-7  ">
        <div className='flex gap-3'>

          <button className="text-xl text-zinc-800 bg-gradient-to-tl from-red-300 to-red-100  border border-red-600 border border-red-600-red-600 px-5 py-1 rounded active:scale-110 font-bold mb-4" onClick={() => setCreateUser(true)}  > Create User  </button>
        </div>

        <table className="min-w-full rounded table-auto border border-red-600-collapse border border-red-600 border border-red-600-gray-300">
          <thead>
            <tr className="bg-red-50 text-zinc-700">
              <th className="border border-red-600 px-4 py-2">Name</th>
              <th className="border border-red-600 px-4 py-2"> Result Update <br /> Permission </th>
              <th className="border border-red-600 px-4 py-2">VIP Number Update <br /> Permission</th>
              <th className="border border-red-600 px-4 py-2">Blocked</th>
            </tr>
          </thead>
          <tbody>
            {isLoading ? <>Loading...</> : createdUser && createdUser.users.map((user, index) => (
              <tr key={user.id} className="text-center">
                <td className="border border-red-600  px-4 py-2">{user.name}</td>
